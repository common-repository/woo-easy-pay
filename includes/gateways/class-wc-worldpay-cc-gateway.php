...
class WC_Online_Worldpay_CC_Payment_Gateway extends WC_Online_Worldpay_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->save_cc_key = $this->id . '_save_cc_key';
	...
	public function process_3ds_order() {
		if (isset ( $_GET[ '_worldpay_3ds_nonce' ] )) {
			$data = WC ()->session->get ( 'worldpay_3ds_data' );
			$order = wc_get_order ( $data[ 'order_id' ] );
			if (! $this->verify_order_nonce ( $_GET[ '_worldpay_3ds_nonce' ], $order )) {
				return;
			}
	...
	/**
	 * Return true if the save credit card checkbox should be visible.
	 * Not visible if this is a request for the add payment method page,
	 * cart contains subscriptions, or this is a request to change a payment
	 * method on a subscription
	 *
	 * @return bool
	 */
	public function display_save_cc() {
		if (is_checkout () && worldpay_cart_has_subscriptions ()) {
			return false;
		}
		if (is_add_payment_method_page ()) {
			return false;
	...
	protected function save_payment_method() {
		return ! $this->use_saved_method () && ! empty ( $_POST[ $this->save_cc_key ] );
	}
}
