...
class WC_Online_Worldpay_Payment_Gateway extends WC_Payment_Gateway {
	...
	public function process_payment($order_id) {
		...
		$args = apply_filters ( 'worldpay_process_payment_args', array( 
				...
				'amount' => worldpay_add_number_precision ( $order->get_total () ), 
		...
		if (empty ( $args[ 'shopperIpAddress' ] )) {
			unset ( $args[ 'shopperIpAddress' ] );
		}
		try {
			$return = array();
			$session = WC ()->session;
			...
			if ($response[ 'paymentStatus' ] === 'SUCCESS') {
				...
				if ($this->is_active ( '3ds_enabled' ) && ! empty ( $response[ 'redirectURL' ] )) {
					$threeDS_data = array( 
							...
							'return_url' => $this->get_3ds_return_url ( $order ) 
					);
	...
	public function admin_options() {
		...
		include worldpay ()->base_path () . 'includes/admin/views/html-notice.php';
	...
	public function maybe_save_payment_token($order_id, $order) {
		if ($order->get_payment_method () === $this->id && $this->should_save_payment_method ( $order )) {
	...
	/**
	 * Return true if the user wants to save the payment method as part of the checkout request.
	 */
	protected function save_payment_method() {
		return false;
	}
	 ...
	 * @return string
	 */
	protected function get_3ds_return_url($order) {
		return add_query_arg ( array( 
				'worldpay_action' => 'process_3ds_order', 
				'_worldpay_3ds_nonce' => $this->create_order_nonce ( $order ) 
	 ...
	 * @param WC_Order $order        	
	 */
	protected function create_order_nonce($order) {
		// process-3ds-order
		return wp_hash ( $order->get_id () . '|' . $order->get_customer_id () );
	}
	 ...
	 * @param WC_Order $order        	
	 */
	protected function verify_order_nonce($nonce, $order) {
		$nonce_2 = $this->create_order_nonce ( $order );
		return hash_equals ( $nonce, $nonce_2 );
	}
