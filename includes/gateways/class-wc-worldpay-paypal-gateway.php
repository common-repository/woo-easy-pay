...
class WC_Online_Worldpay_PayPal_Gateway extends WC_Online_Worldpay_Payment_Gateway {
	...
	public function payment_args($args, $order, $gateway_id) {
		if ($this->id === $gateway_id) {
			$base_url = trailingslashit ( get_site_url () ) . 'index.php';
			$nonce = wp_create_nonce ( 'paypal-action' );
			$args[ 'successUrl' ] = add_query_arg ( array( 
					'worldpay_action' => 'paypal_success', 
					'_paypal_nonce' => $nonce 
			), $base_url );
			$args[ 'pendingUrl' ] = add_query_arg ( array( 
					'worldpay_action' => 'paypal_pending', 
					'_paypal_nonce' => $nonce 
			), $base_url );
			$args[ 'failureUrl' ] = add_query_arg ( array( 
					'worldpay_action' => 'paypal_failure', 
					'_paypal_nonce' => $nonce 
			), $base_url );
			$args[ 'cancelUrl' ] = add_query_arg ( array( 
					'worldpay_action' => 'paypal_cancel', 
					'_paypal_nonce' => $nonce 
			), $base_url );
		}
		return $args;
